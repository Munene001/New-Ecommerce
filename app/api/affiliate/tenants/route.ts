// app/api/affiliate/tenants/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Stats for this affiliate (their referred tenants)
    const [statsResult] = await pool.query<RowDataPacket[]>(
      `SELECT 
        COUNT(*) as total_tenants,
        SUM(CASE WHEN account_status = 'free_trial' THEN 1 ELSE 0 END) as free_trial,
        SUM(CASE WHEN account_status = 'active' THEN 1 ELSE 0 END) as active,
        SUM(CASE WHEN account_status IN ('expired', 'suspended') THEN 1 ELSE 0 END) as expired_suspended
       FROM tenant
       WHERE affiliate_id = ?`,
      [affiliateId]
    );
    ...
    console.error('GET affiliate tenants error:', error);
