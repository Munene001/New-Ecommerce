// app/(dashboard)/[shopSlug]/sales/[saleId]/page.tsx
'use client';
...
export default function IndividualSale({ params }: PageProps) {
  const router = useRouter();
  const [shopSlug, setShopSlug] = useState<string>("");
  const [saleId, setSaleId] = useState<number | null>(null);
  const [sale, setSale] = useState<any>(null);
  const [items, setItems] = useState<SaleItemWithImage[]>([]);
  ...
  useEffect(() => {
    const loadParams = async () => {
      const { shopSlug: slug, saleId: id } = await params;
      setShopSlug(slug);
      setSaleId(parseInt(id));
    };
    loadParams();
  }, [params]);
  ...
  const fetchSaleDetails = async () => {
    ...
    if (result) {
      ...
      const itemsWithImages = await Promise.all(
        result.items.map(async (item: SaleItem) => {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    const success = await updateOrderStatus(saleId, newStatus);
    if (success) {
      setSale({ ...sale, order_status: newStatus });
      setMessage({ type: 'success', text: `Sale status updated to ${newStatus}` });
    } else {
      setMessage({ type: 'error', text: 'Failed to update sale status' });
    }
  };
  ...
  const handleDeleteSale = async () => {
    ...
    const success = await deleteOrder(saleId);
    if (success) {
      ...
      setMessage({ type: 'error', text: 'Failed to delete sale' });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700">Sale not found</h2>
          <Button
            onClick={() => router.back()}
            variant="secondary"
            className="mt-4"
    ...
    <div className="md:p-6 px-4 py-6 font-[Poppins] max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-gray-800">Sale #{sale.order_number}</h1>
            <p className="text-gray-500 text-sm mt-1">Placed on {formatDate(sale.created_at)}</p>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          {sale.special_instructions && (
            <div className="bg-white rounded-lg border border-gray-300">
              ...
              <div className="px-6 py-4">
                <p className="text-gray-700">{sale.special_instructions}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-300">
            ...
            <div className="px-6 py-4 space-y-3">
              ...
              <div>
                ...
                <p className="text-gray-700">{sale.customer_phone}</p>
              </div>
              <div>
                ...
                <p className="text-gray-700">{sale.customer_address}</p>
                <p className="text-gray-500 text-sm">{sale.customer_city}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-300">
            <div className="px-6 py-4 border-b border-gray-300 bg-gray-100">
              <h2 className="font-semibold text-gray-800">Sale Status</h2>
            </div>
            <div className="px-6 py-4">
              <select
                value={sale.order_status}
                onChange={(e) => handleStatusChange(e.target.value)}
                className={`w-full px-3 py-2 rounded-lg border font-medium cursor-pointer ${getStatusColor(sale.order_status)}`}
              >
            ...
            <div className="px-6 py-4 space-y-3">
              <div>
                ...
                <p className="font-medium text-gray-800 capitalize">
                  {sale.payment_method === 'cash_on_delivery' ? 'Cash on Delivery' : sale.payment_method}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-300">
            ...
            <div className="px-6 py-4 space-y-3">
              ...
              <div>
                ...
                <p className="text-gray-700">{formatDate(sale.updated_at)}</p>
              </div>
            </div>
          </div>
        </div>
