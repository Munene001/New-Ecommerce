...
import { KopokopoStatus } from "./stk-ui/kopokopoStatus";
...
interface KopokopoPaymentProps {
  orderId: string | null;
  order: Order | null;
...
export function KopokopoPayment({ 
  orderId, 
  order, 
  initialSavedState,
  onPaymentSuccess,
  onStateChange 
}: KopokopoPaymentProps) {
  ...
  const handleKopokopoPush = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/shops/payments/kopokopo', {
        method: 'POST',
        headers: {
      ...
      console.error('Kopokopo payment error:', error);
      setPaymentStatus('failed');
      setRetryable(true);
